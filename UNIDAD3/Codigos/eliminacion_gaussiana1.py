...
def verificar_solucion(A, b, x):
    """
    Verifica la solucion sustituyendo x en Ax = b.

    Calcula el producto punto A[i] · x para cada fila i
    y lo compara con b[i]. Si la diferencia es menor a
    1e-8 se considera correcto (tolerancia numerica).
    """
    print("\n [VERIFICACION] Sustituyendo solucion en Ax = b:")
    todos_ok = True
    for i in range(len(b)):
        ...
        if ok == "ERROR":
            todos_ok = False
        print(f"   Ecuacion {i+1}: {resultado:.6f} = {b[i]}  [{ok}]")
    if todos_ok:
        print("\n   Todas las ecuaciones se verifican correctamente.")
